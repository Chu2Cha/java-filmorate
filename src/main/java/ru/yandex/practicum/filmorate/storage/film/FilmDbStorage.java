...
public class FilmDbStorage implements FilmStorage {
    ...
    public Film updateFilm(Film film) {
        String sql = "UPDATE FILM SET FILM_NAME = ?, DESCRIPTION = ?, FILM_RELEASE_DATE = ?, FILM_DURATION = ?, MPA_ID = ?";
        jdbcTemplate.update(
                sql, film.getName(), film.getDescription(), film.getReleaseDate(), film.getDuration(), film.getMpa().getId()
        );
        updateGenre(film);
        return film;
    }
    ...
    private void updateGenre(Film film) {
        if (film.getGenres() != null) {
            for (Genre genre : film.getGenres()) {
                jdbcTemplate.update("UPDATE FILM_GENRE SET FILM_ID = ?, GENRE_ID = ?",
                        film.getId(), genre.getId());
            }
        }
    }
